  ...
  class SweepstakeWheelV2 {
    ...
    spin() {
      ...
      if (self.isAdmin) {
        ...
        var _token = localStorage.getItem('tc_token');
        var _headers = { 'Content-Type': 'application/json' };
        if (_token) _headers['Authorization'] = 'Bearer ' + _token;
        fetch('/api/admin/sweep/' + self.sweepstakeId + '/draw', {
          method: 'POST',
          headers: _headers
        })
        ...
        .then(function(result) {
          ...
          var selected = null;
          var swId = serverWinner.user_id || serverWinner.userId;
          for (var i = 0; i < self.entries.length; i++) {
            if (String(self.entries[i].id) === String(swId)) {
              selected = self.entries[i];
              break;
            }
          }
          if (!selected) {
            selected = { id: swId, name: serverWinner.name || serverWinner.displayName || 'Unknown', entries: serverWinner.entries || 1 };
          }
    ...
    animateToWinner(selectedWinner) {
      ...
      for (var i = 0; i < self.entries.length; i++) {
        ...
        if (String(self.entries[i].id) === String(selectedWinner.id)) {
    ...
    replay() {
      ...
      if (replayId) {
        var winner = null;
        for (var i = 0; i < self.entries.length; i++) {
          if (String(self.entries[i].id) === String(replayId)) { winner = self.entries[i]; break; }
        }
        if (winner) {
          setTimeout(function() {
            self.isSpinning = true;
