...
(function() {
  ...
  class SweepSectionV2 {
    ...
    async loadAll() {
      ...
      try {
        var resp = await fetch('/api/sweep/active', { headers: this._tcHeaders() });
        var json = await resp.json();
        var s = json.sweep;
        if (!s) {
          this.otherSweepstakes = [];
          this.data = { sweepstake: {}, user: { entries: 0, balance: 0 }, prize: {}, donor: {}, winner: null };
          this.render();
          return;
        }
        this.otherSweepstakes = [];
        var hasWinner = s.status === 'completed' && s.winner_user_id;
        this.data = {
          sweepstake: {
            id: s.id, status: s.status, title: s.title, prize: s.prize,
            description: s.prize_description, imageUrl: s.prize_image_url,
            estimatedValue: s.prize_value ? '$' + Number(s.prize_value).toFixed(0) : '',
            category: 'Prize', endAt: s.end_at, entryCost: s.entry_cost || 1,
            maxEntriesPerDay: s.max_entries_per_user_per_day || 10
          },
          totals: { totalEntries: s.total_entries || 0 },
          user: { entries: s.user_entries || 0, balance: s.user_balance || 0 },
          prize: {
            title: s.prize, description: s.prize_description,
            imageUrl: s.prize_image_url,
            valueCents: s.prize_value ? Math.round(Number(s.prize_value) * 100) : 0
          },
          donor: s.donor_name ? { businessName: s.donor_name } : {},
          winner: hasWinner ? { name: 'Winner', userId: s.winner_user_id } : null,
          isUpcoming: false
        };
        this.render();
      } catch (e) {
    ...
    handleShare() {
      ...
      if (sweep?.id) {
        ...
        shareUrl = window.location.origin + '/giveaway';
        shareText = 'Check out the Treasure Coast community giveaway — earn points and win prizes!';
    ...
    async logShare(platform) {
      // Share logging not implemented for TC yet
    }

    async handleEnter(amount) {
      var sweepId = this.data?.sweepstake?.id;
      if (!sweepId) return;
      try {
        var resp = await fetch('/api/sweep/enter', {
          method: 'POST',
          headers: Object.assign({ 'Content-Type': 'application/json' }, this._tcHeaders()),
          body: JSON.stringify({ sweepId: sweepId, count: amount || 1 })
        });
        ...
        alert('Entered ' + data.entered + 'x! Balance: ' + data.new_balance + ' pts');
        await this.loadAll();
      } catch (e) {
    ...
    async loadRules() {
      // TC uses static rules display — no public rules endpoint needed
      this.rules = [
        { eventType: 'listing_create', amount: 10 },
        { eventType: 'purchase', buyerAmount: 5 },
        { eventType: 'review_left', amount: 3 },
      ];
    }
            ...
            <div class="sweep-v2-prize-grid">
              ...
              <div class="sweep-v2-prize-details">
                ...
                <div class="sweep-v2-entry-section">
                  ...
                  <div class="sweep-v2-entry-note">Each entry costs ${sweep.entryCost || 1} point${(sweep.entryCost || 1) > 1 ? 's' : ''} from your balance</div>
                </div>
                ` : `
                <div class="sweep-v2-entry-section">
                  <a href="/login" class="sweep-v2-entry-btn" style="text-decoration:none;text-align:center;display:block;">
                    \uD83C\uDF9F\uFE0F Log In to Enter
                  </a>
                  <div class="sweep-v2-entry-note">Log in or create an account to enter this giveaway</div>
                </div>
                `
                ) : ''}
              </div>
    ...
    bindEvents() {
      ...
      if (wheelBtn) {
        wheelBtn.addEventListener('click', async () => {
          try {
            var sweepId = this.data?.sweepstake?.id;
            if (!sweepId) { window.sweepWheelV2?.open([], null); return; }
            const resp = await fetch('/api/sweep/leaderboard?sweepId=' + sweepId);
            const data = await resp.json();
            const entries = (data.participants || []).map(p => ({
              id: p.id,
              name: p.name || 'Unknown',
              entries: p.total_entries || 1
            }));
            const isAdmin = !!(window._tcUser && window._tcUser.role === 'admin');
            const winnerId = this.data?.winner ? this.data.winner.userId : null;
