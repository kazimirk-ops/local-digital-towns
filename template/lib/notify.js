...
async function sendEmail(to, subject, text, html){
  ...
  if(!apiKey || !toList.length){
    console.warn("Email notify not configured (missing RESEND_API_KEY or recipient)");
    return { ok: false, skipped: true, statusCode: null };
  }
  ...
  try{
    ...
    if(!resp.ok){
      const errText = await resp.text().catch(() => "");
      console.error("EMAIL_SEND_ERROR", { statusCode: resp.status, error: errText });
      return { ok: false, error: errText || `Resend error ${resp.status}`, statusCode: resp.status };
    }

    const result = await resp.json().catch(() => ({}));
    console.log("EMAIL_SEND_SUCCESS", { statusCode: resp.status, id: result.id });
    return { ok: true, statusCode: resp.status, id: result.id };
  }catch(err){
    console.error("EMAIL_SEND_ERROR", { error: err.message || String(err) });
    return { ok: false, error: err.message || String(err), statusCode: null };
  }
}
