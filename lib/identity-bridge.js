...
async function issueSSO(db, { email, platformSlug, platformUserId, redirectTo }) {
  var crypto = require('crypto');
  var token = crypto.randomBytes(32).toString('hex');
  await db.query(
    "INSERT INTO sso_tokens " +
    "(token, email, platform_slug, platform_user_id, redirect_to, expires_at) " +
    "VALUES ($1, $2, $3, $4, $5, NOW() + INTERVAL '5 minutes')",
    [token, email.toLowerCase().trim(), platformSlug, platformUserId || null, redirectTo || null]
  );
  return token;
}

module.exports = { linkPlatformUser, issueSSO };
